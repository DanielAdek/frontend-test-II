...
class Gif extends Component {
    ...
    const { chatText } = this.state;
        return (
            ...
            {this.state.images.length < 1 ? <h5 style={{textAlign: 'center'}}>Enter text in the input below!</h5> : this.state.images.map((image, i) => 
                <div key={ i }>
                    <img 
                        style={{width: '10rem', maxWidth: '300px', height: '10rem', maxHeight: '200px'}}
                        src={`${image}`}/> 
                </div>
            ) }
            </div>
